                ...
                if (ingredientList.size() == 9) {
                    ...
                    if (ingredientEquals.size() == 1) {
                        for (final ItemStack ingredient : ingredientEquals.keySet()) {
                            ...
                            final ItemStack result = recipe.getResult();
                            ItemCompress.INSTANCE.getLogger().info("忽略无序配方: " + material + " x9 -> " + result.getType() + " x" + result.getAmount());
                        }
                    }
                }
                continue;
                ...
                if (ingredientMap.size() == 9) {
                    ...
                    if (ingredientEquals.size() == 1) {
                        for (final ItemStack ingredient : ingredientEquals.keySet()) {
                            ...
                            compressMaterial.remove(material);
                            final ItemStack result = recipe.getResult();
                            ItemCompress.INSTANCE.getLogger().info("忽略有序配方: " + material + " x9 -> " + result.getType() + " x" + result.getAmount());
                        }
                    }
                }
            }
