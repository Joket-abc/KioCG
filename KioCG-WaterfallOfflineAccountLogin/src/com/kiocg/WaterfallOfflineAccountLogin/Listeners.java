...
public class Listeners implements Listener {
    ...
    public void onPreLogin(final PreLoginEvent e) {
        ...
        if (!Pattern.matches("^[0-9a-zA-Z_]{3,16}$", playerName)) {
            e.setCancelReason(new ComponentBuilder("\n\n\n\n... ").color(ChatColor.GRAY).append("名称只能由3-16位字母数字下划线组成").color(ChatColor.RED).append(" ...").color(ChatColor.GRAY)
                                                                  .append("\n\n\n\n\n\n\n\n\n\n\n").append("(WaterfallOfflineAccountLogin)").color(ChatColor.DARK_GRAY).create());
            e.setCancelled(true);
            return;
        }
